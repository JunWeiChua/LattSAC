classdef UnitCellStrut < handle
    ...
    methods
		...
		function calcDelta(unitCell)
			...
			X = [unitCell.cell_length unitCell.rel_density];
			X = [X sum(X.^2,2)];
			X_n = ( X - unitCell.CellArch.Delta2Model.mean ) ./ unitCell.CellArch.Delta2Model.std;
			Y = [];
			for i = 1:length(unitCell.CellArch.Delta2Model.NN)
				model = unitCell.CellArch.Delta2Model.NN{i};
				Y(i) = model(X_n');
			end
